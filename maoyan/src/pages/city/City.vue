  ...
  <div>
    <section id="city-list" class="city-list-container" style="display: block;">
      ...
      <section class="nav">
        ...
        <div class="nav-item" id="handlebar_hot" @click="scrollToElement('#hot')">
          热门
        </div>

        <div class="nav-letter nav-item"
          @click="scrollToElement(`#${key}`)"
          v-for="(alphabeta, key) in cities"
          :key="key"
...
export default {
  ...
  methods: {
    ...
    handleTouchMove(e) {
      if(this.touch) {
        ...
        let navLetterIndex = Math.floor((clientY - this.hotposition) / this.alphabetaHeight)
  ...
  async mounted() {
    ...
    this.hotposition = this.$refs['A'][0].getBoundingClientRect()['y']
