<template>
  <div class="most-expected">
    ...
    <div class="most-expected-list">
      <div>
        <div class="expected-item" v-for="m in list" :key="m.id">
          <div class="poster default-img-bg">
            <img :src="m.img | replace('170.230')" onerror="this.style.visibility='hidden'" />
  ...
  methods: {
    ...
    scroll() {
      ...
      bScroll.on("scrollEnd", async function() {
        if (that.isFinished) {
          // 为了防止没有完成ajax数据时，用户频繁拖拽再次请求数据
          that.isFinished = false;
          // 当用户拖拽不到滚动条尾部时 当前页面要在前3页内 (接口只有3页数据) ，不能触发ajax请求
          if (this.maxScrollX - this.x >= 0 && that.page < 3) {
            ...
            // 当新页数据加载完毕后，才做page的更新。目的是为了Toast在滚动到第三页时在现实
            that.$nextTick(() => {
              that.page++
