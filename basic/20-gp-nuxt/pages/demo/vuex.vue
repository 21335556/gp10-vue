<template>
  <div class="container">
    ...
    <ul>
      <li v-for="movie in movies" :key="movie.id" @click="handleClick(movie.id)">{{movie.nm}}</li>
    </ul>
  </div>
</template>
...
export default {
  async asyncData({$axios}){
    ...
    this.$store.commit('demo/loadMovies', result)
  },
  ...
  computed: {
    ...
    count() {
      return this.$store.store.demo.count
    },
    movies() {
